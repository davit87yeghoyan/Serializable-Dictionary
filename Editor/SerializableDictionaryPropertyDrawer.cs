...
namespace Serializable_Dictionary.Editor
{
    ...
    public class SerializableDictionaryPropertyDrawer : PropertyDrawer
    {
        ...
        public static void OnGUIDraw(Rect position, SerializedProperty property, GUIContent label, SerializableDictionaryDrawerSettingAttribute setting = null)
        {
            ...
            if (property.isExpanded)
            {
                ...
                foreach (var entry in EnumerateEntries(keyArrayProperty, valueArrayProperty))
                {
                    ...
                    float lineHeight = DrawKeyValueLine(keyProperty, valueProperty, linePosition, i,setting);
        ...
        static float DrawKeyValueLine(SerializedProperty keyProperty, SerializedProperty valueProperty, Rect linePosition, int index,SerializableDictionaryDrawerSettingAttribute setting = null)
        {
            ...
            if (valueProperty != null)
            {
                ...
                {
                    return DrawKeyValueLineExpand(keyProperty, valueProperty, linePosition,setting);
                }
                else
                {
                    var keyLabel = keyCanBeExpanded ? ("Key " + index.ToString()) : "";
                    var valueLabel = valueCanBeExpanded ? ("Value " + index.ToString()) : "";
                    return DrawKeyValueLineSimple(keyProperty, valueProperty, keyLabel, valueLabel, linePosition,setting);
                }
            }
            else
            {
        ...
        static float DrawKeyValueLineSimple(SerializedProperty keyProperty, SerializedProperty valueProperty, string keyLabel, string valueLabel, Rect linePosition,SerializableDictionaryDrawerSettingAttribute setting = null)
        {
        ...
        private static bool KeyDisable(SerializedProperty keyProperty,  SerializableDictionaryDrawerSettingAttribute setting, ref Rect keyPosition, float labelWidth)
        {
            ...
            if (staticMethodName != null && type != null)
            {
                MethodInfo builderMethod = type.GetMethod(staticMethodName);
                if (builderMethod is { })
                {
            ...
            if (keyDisable)
            {
                var keyPositionXMin = 40;
                keyPosition.width = keyPositionXMin;
                ...
                keyPosition.width = labelWidth - IndentWidth;
                keyPosition.xMin += keyPositionXMin - 10;
            }
        ...
        static float DrawKeyValueLineExpand(SerializedProperty keyProperty, SerializedProperty valueProperty, Rect linePosition,
            SerializableDictionaryDrawerSettingAttribute setting)
        {
            float labelWidth = EditorGUIUtility.labelWidth;
    ...
    [CustomPropertyDrawer(typeof(SerializableDictionaryDrawerSettingAttribute), true)]
    public class SerializableDictionaryDrawerSetting : PropertyDrawer
    {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
             SerializableDictionaryDrawerSettingAttribute settingAttribute = attribute as SerializableDictionaryDrawerSettingAttribute;
             SerializableDictionaryPropertyDrawer.OnGUIDraw(position,property,label,settingAttribute);
        }
        
        public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
        {
            return  SerializableDictionaryPropertyDrawer.PropertyHeight(property);
        }
    }
